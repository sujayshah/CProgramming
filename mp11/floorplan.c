 ...
 * ----------------------
 * In this MP, I implemented a VLSI floorplanner to optimise the arrangement of specific modules on a chip. This algorithm worked by iterating from an initial configuration with all the modules
 * lined up vertically next to each other. New solutions were generated by using various manipulation operations. The area of each new iteration was then compared with the previous one. If it
 * was lower, we moved to the new solution. Otherwise, we decide whether or not to move to this solution using some pre-defined probability. The manipulations implemented were: rotate, recut,
 * swap module and swap topology. Each of these functions operated on a binary tree representation of the chip layout. In addition, the function 'get expression' obtained the postfix 
 * representation of the binary tree. Finally, the 'initial slicing tree' function creates the binary tree using arguments passed by an input read from a file.
*/
...
int is_in_subtree(node_t* a, node_t* b)                                                                      
{
  // TODO: (remember to modify the return value appropriately)	
	...
	if(a->cutline != UNDEFINED_CUTLINE)
	{
...
void swap_topology(node_t* a, node_t* b)                                                
{
  ...
  // TODO:
	...
	if((a->parent)->left == a) //check which side of the parent a is on
	{
		flagA = 1; //indicates that a is on the left of its parent
	}
	else
	{
		flagA = 0; //indicates that a is on the right of its parent
	}

	if((b->parent)->left == b) //check which side of the parent b is on
	{
		flagB = 1; //indicates that b is on the left of its parent
	}
	else
	{
		flagB = 0; //indicates that b is on the right of its parent
	}

	if(flagA == 1)
	{
		(a->parent)->left = b; //set b to be on the left of a's parent
		b->parent = a->parent; //update parent pointer
	}
	else
	{
		(a->parent)->right = b; //set b to be on the right of a's parent
		b->parent = a->parent;
	}
	
	if(flagB == 1)
	{
		temp1->left = temp;     //set a to be on the left of b's parent
		a->parent = temp1;      //update parent pointer
	}	
	else
	{
		temp1->right = temp;    //set a to be on the right of b's parent
		a->parent = temp1;
	}	
}
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression)                     
{
  ...
  // TODO:
	...
	postfix_traversal(ptr->left, nth, expression);  //traverse left subtree  
	postfix_traversal(ptr->right, nth, expression); //traverse right subtree
	
	expression[*nth].cutline = ptr->cutline; //fill data
...
//
// In each recursive step of this function, you are passed a parent pointer pointing to the
// parent node of the node that you will generate at this step, as well as an integer index n 
...
{
	assert(n >= 0 && n < num_modules);                                                 //WRONG!

  // TODO: (remember to remove the following return statement)

	node_t * root_node = (node_t *)malloc(sizeof(node_t)); //allocate memory for new parent

	if(n == num_modules-1) //base case
	{
		root_node->cutline = UNDEFINED_CUTLINE;
		root_node->module = &modules[n];
		root_node->parent = par;
	...
	{
		node_t * new_right = (node_t *)malloc(sizeof(node_t)); //allocate memory for right node
		...
		root_node->cutline = V;
		root_node->module = NULL;
		root_node->parent = par;
		root_node->right = new_right;
		root_node->left = init_slicing_tree(root_node, n+1);
